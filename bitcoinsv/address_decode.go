...
import (
	...
	"github.com/blocktree/bitcoinsv-adapter/bitcoinsv_addrdec"
...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := bitcoinsv_addrdec.BSV_mainnetPrivateWIFCompressed
	if decoder.wm.Config.IsTestNet {
		cfg = bitcoinsv_addrdec.BSV_testnetPrivateWIFCompressed
	}

	wif, _ := bitcoinsv_addrdec.Default.AddressEncode(priv, cfg)
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	bitcoinsv_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	address, err := bitcoinsv_addrdec.Default.AddressEncode(pub)
	if err != nil {
		return "", err
	}
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := bitcoinsv_addrdec.BSV_mainnetAddressP2SH
	if decoder.wm.Config.IsTestNet {
		cfg = bitcoinsv_addrdec.BSV_testnetAddressP2SH
	}
	...
	address, _ := bitcoinsv_addrdec.Default.AddressEncode(pkHash, cfg)
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := bitcoinsv_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
