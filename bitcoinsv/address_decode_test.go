...
func TestAddressDecoder_AddressEncode(t *testing.T) {
	bitcoinsv_addrdec.Default.IsTestNet = false

	p2pk, _ := hex.DecodeString("8d7b97fd0ff59003185e0ab94d46b1c91818f901")
	p2pkAddr, _ := bitcoinsv_addrdec.Default.AddressEncode(p2pk)
	t.Logf("p2pkAddr: %s", p2pkAddr)

	p2sh, _ := hex.DecodeString("131a861f0609944596e2d618e41ba8ce07b281d0")
	p2shAddr, _ := bitcoinsv_addrdec.Default.AddressEncode(p2sh, bitcoinsv_addrdec.BSV_mainnetAddressP2SH)
...
func TestAddressDecoder_AddressDecode(t *testing.T) {
	...
	p2pkHash, _ := bitcoinsv_addrdec.Default.AddressDecode(p2pkAddr)
	t.Logf("p2pkHash: %s", hex.EncodeToString(p2pkHash))

	//p2shAddr := "sQMG5PncvvxVMrVwXpFfBoi3JFHvPiA9aw"
	//
	//p2shHash, _ := bitcoinsv_addrdec.Default.AddressDecode(p2shAddr, bitcoinsv_addrdec.BSV_mainnetAddressP2SH)
