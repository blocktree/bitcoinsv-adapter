...
type BSVBlockScanner struct {
	*bitcoin.BTCBlockScanner
	dscWM *WalletManager
}

//NewBTCBlockScanner 创建区块链扫描器
func NewBSVBlockScanner(wm *WalletManager) *BSVBlockScanner {
	bs := &BSVBlockScanner{dscWM: wm}
...
func (bs *BSVBlockScanner) GetBalanceByAddress(address ...string) ([]*openwallet.Balance, error) {
