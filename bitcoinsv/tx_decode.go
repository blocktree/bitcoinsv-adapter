...
import (
	...
	"github.com/blocktree/go-owcdrivers/bitcoinsvTransaction"
...
func (decoder *TransactionDecoder) SignBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			txHash := bitcoinsvTransaction.TxHash{
				Hash: keySignature.Message,
				Normal: &bitcoinsvTransaction.NormalTx{
					Address: keySignature.Address.Address,
					SigType: bitcoinsvTransaction.SigHashAll,
				},
			}
			...
			//签名交易
			/////////交易单哈希签名
			sigPub, err := bitcoinsvTransaction.SignRawTransactionHash(txHash.GetTxHashHex(), keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	var (
		txUnlocks  = make([]bitcoinsvTransaction.TxUnlock, 0)
		emptyTrans = rawTx.RawHex
		//sigPub     = make([]bitcoinsvTransaction.SignaturePubkey, 0)
		transHash     = make([]bitcoinsvTransaction.TxHash, 0)
		addressPrefix bitcoinsvTransaction.AddressPrefix
	)
	...
	for accountID, keySignatures := range rawTx.Signatures {
		...
		for _, keySignature := range keySignatures {
			...
			signaturePubkey := bitcoinsvTransaction.SignaturePubkey{
				Signature: signature,
				Pubkey:    pubkey,
			}
	...
	trx, err := bitcoinsvTransaction.DecodeRawTransaction(txBytes, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	for _, vin := range trx.Vins {
		...
		txUnlock := bitcoinsvTransaction.TxUnlock{
	...
	signedTrans, err := bitcoinsvTransaction.InsertSignatureIntoEmptyTransaction(emptyTrans, transHash, txUnlocks, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	pass := bitcoinsvTransaction.VerifyRawTransaction(signedTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if pass {
...
func (decoder *TransactionDecoder) createBTCRawTransaction(
	...
	var (
		err              error
		vins             = make([]bitcoinsvTransaction.Vin, 0)
		vouts            = make([]bitcoinsvTransaction.Vout, 0)
		txUnlocks        = make([]bitcoinsvTransaction.TxUnlock, 0)
		totalSend        = decimal.New(0, 0)
		destinations     = make([]string, 0)
		accountTotalSent = decimal.Zero
	...
	for _, utxo := range usedUTXO {
		in := bitcoinsvTransaction.Vin{utxo.TxID, uint32(utxo.Vout)}
		vins = append(vins, in)

		amount, _  := decimal.NewFromString(utxo.Amount)
		amount = amount.Shift(decoder.wm.Decimal())

		txUnlock := bitcoinsvTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: bitcoinsvTransaction.SigHashAll, Amount:uint64(amount.IntPart())}
		txUnlocks = append(txUnlocks, txUnlock)
	...
	for to, amount := range to {
		...
		amount = amount.Shift(decoder.wm.Decimal())
		out := bitcoinsvTransaction.Vout{to, uint64(amount.IntPart())}
	...
	/////////构建空交易单
	emptyTrans, err := bitcoinsvTransaction.CreateEmptyRawTransaction(vins, vouts, lockTime, replaceable, addressPrefix)
	...
	////////构建用于签名的交易单哈希
	transHash, err := bitcoinsvTransaction.CreateRawTransactionHashForSig(emptyTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if err != nil {
...
func (decoder *TransactionDecoder) createOmniRawTransaction(
	...
	for _, utxo := range usedUTXO {
		...
		txUnlock := omniTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: bitcoinsvTransaction.SigHashAll}
		txUnlocks = append(txUnlocks, txUnlock)
